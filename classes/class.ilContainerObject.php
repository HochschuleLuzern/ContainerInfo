...
{	
	...
	private function getSizeOfMcst($obj_id)
	{
		...
		$query = sprintf("SELECT mob_id FROM il_news_item ".
	...
	private function getSizeOfMep($obj_id)
	{
		...
		// I didn found any lookup-functions for this...
		$query = sprintf("SELECT foreign_id FROM mep_tree " .
	...
	private function checkForReadEvents($obj_id)
	{
		 ...
		 */
		global $ilDB;
		$query = sprintf("SELECT obj_id, usr_id, last_access, read_count FROM read_event ".
	 ...
	 */
	static function getChildRefIdsFromParentRefId($parent_ref)
	{
		global $tree;
	 ...
	 */
	static function getChildContainerFromParentRefId($parent_ref)
	{
		$children = array();
		$i = 0;
	 ...
	 */	
	static function compareContainerForSort($a, $b)
	{
